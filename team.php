    ...
    <?php if ($recordSet->fields['amplua'] != 4 && $recordSet->fields['amplua'] != 5):?>
      ...
      <div id="playerCard<?= $idx ?>" class="card-player-full content-image">
        ...
        <div class="card-player-full__photo">
          ...
          <?  if($kep) faceupload($recordSet2->fields[man], $id, $tournament); ?>
          ...
          <? if($kep){ ?>
            ...
            <span>Амплуа
              <select class='form-control' id='amplua_<?=$recordSet2->fields[id];?>' size=1 onchange='amplua(<?=$recordSet2->fields[id];?>, <?=$recordSet2->fields[man];?>);'>
                <option value='0' <? if($recordname->fields[amplua]==0) echo "selected";?>>-</option>
                <option value='1' <? if($recordname->fields[amplua]==1) echo "selected";?>>Нападник</option>
                <option value='2' <? if($recordname->fields[amplua]==2) echo "selected";?>>Захисник</option>
                <option value='3' <? if($recordname->fields[amplua]==3) echo "selected";?>>Воротар</option>
              </select>
            </span> 
          <? } ?>
        </div>
    ...
    <!-- ///////////////////////// -->
      ...
      <table class="sostav-tab">  
        <?
          ...
          while (!$recordSet->EOF) {
            ...
            if ($recordface->fields[pict]) $face = $recordface->fields[pict]; else $face = "avatar1.jpg";
            if($managerAndTrainer->fields['manager'] != $recordSet->fields['id'] && $managerAndTrainer->fields['trainer'] != $recordSet->fields['id'] ){
        ?>
      ...
      <tr>
        <td>
          ...
          <?  if($kep) faceupload($recordSet->fields[man], $id, $tournament); ?>
        </td>
        <td>
            <a href="<?=$site_url?>/<?=$tournament?>/player/info/<?=$recordSet->fields[man]?>">
        ...
        <td>
          <? if ($kep){if ($recordSet->fields[active] == 1) {echo"<font color='green'>Заявлений</font></td><td>";
